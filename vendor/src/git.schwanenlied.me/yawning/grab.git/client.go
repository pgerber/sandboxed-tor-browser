...
func (c *Client) do(req *Request) (*Response, error) {
	...
	for _, method := range methods {
		...
		if hresp, err := c.HTTPClient.Do(req.HTTPRequest); err != nil {
			...
			if toMemory {
				// Preallocate the buffer if possible, resetting the state as
				// needed.
				if req.Buffer.Len() > 0 {
					req.Buffer.Reset()
				}
				if resp.Size > 0 && resp.Size <= math.MaxInt32 {
					// XXX/Yawning: Yes this will cause issues on 32 bit
					// systems, but Tor Browser downloads will never grow that
					// large, and this is a specific hack for my uses.
					req.Buffer.Grow(int(resp.Size))
				}
			} else {
	...
	if toMemory {
		resp.writer = &stubWriteCloser{bufio.NewWriter(req.Buffer)}
	} else {
